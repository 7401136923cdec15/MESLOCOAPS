...
public class APSManualUtils {
	private static Logger logger = LoggerFactory.getLogger(APSManualUtils.class);

	public APSManualUtils() {
	}

	private static APSManualUtils Instance;

	public static APSManualUtils getInstance() {
		if (Instance == null)
			Instance = new APSManualUtils();
		return Instance;
	}
...
//			APSManuCapacity wManuCapacity = null;
			...
			// 根据已排计划检查其工位加工负荷
			wResult.addAll(APSWorkTimeCalcUtils.getInstance().CheckBuildTask(wCheckTaskList, wManuCapacityList));
...
//					Double wMinus = APSWorkTimeCalcUtils.getInstance().CalcIntervalTime(wLoginUser, wManuCapacity.PartID,
//							wWorkDay, (Calendar) wBuildAPSTaskPart.StartTime.clone(),
//							(Calendar) wBuildAPSTaskPart.EndTime.clone(), wAllZoneList, wCalendarMap);
//					if ((wUndoAPSTaskPart.WorkHour + wMinus) < wManuCapacity.WorkHour) {
//						wMsg = new APSMessage();
//						wMsg.ID = 0;
...
//										.findFirst();
								...
								wOptional = wOrderPartIssuedList.stream()
	...
	private List<APSMessage> CheckOrderPlantDate(List<APSTaskPart> wCheckTaskList, OMSOrder wOrder) {
		...
		for (APSTaskPart wAPSTaskPart : wCheckTaskList) {
			...
			wMsg.MessageText = StringUtils.Format("计划进厂时间【{0}】，计划开始时台车未进厂",
