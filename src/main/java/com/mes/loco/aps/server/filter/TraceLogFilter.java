package com.mes.loco.aps.server.filter;
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			try {
				...
				if (status == 400 && request.getMethod().equalsIgnoreCase("POST")) {

					List<String> wHeaderStringList = new ArrayList<String>();
					Enumeration<String> headerNames = request.getHeaderNames();
					while (headerNames.hasMoreElements()) {
						String name = headerNames.nextElement();
						Enumeration<String> wHeaders = request.getHeaders(name);
						name += ":";
						while (wHeaders.hasMoreElements()) {
							name += wHeaders.nextElement() + " _ ";
						}
						if (name.endsWith(" _ "))
							name = name.substring(0, name.length() - 3);

						wHeaderStringList.add(name);
					}

					String wHeaderString = StringUtils.Join(" ; ", wHeaderStringList);
					// 打印整个请求头与请求体
					logger.error("Request Bad 400;ProjectName:" + BaseController.GetProjectName(request) + "; URI:"
							+ request.getRequestURI() + "; Headers:[" + wHeaderString + "]; ContentType:"
							+ request.getContentType() + " RequestBody:" + getRequestBody(request));
				}
...
//								System.currentTimeMillis() - startTime, status); // updateResponse(res);
						...
						LoggerTool.SaveApiLog(wBMSEmployee.CompanyID, wBMSEmployee.ID,
								BaseController.GetProjectName(request), path, request.getMethod(),
								request.getMethod().equalsIgnoreCase("POST") ? request.getContentType()
										: JSON.toJSONString(request.getParameterMap()),
								request.getMethod().equalsIgnoreCase("POST") ? getRequestBody(request)
