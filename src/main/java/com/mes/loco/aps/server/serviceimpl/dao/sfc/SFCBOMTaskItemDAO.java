		...
		try {
			...
			if (wSFCBOMTaskItem.getID() <= 0) {
				wSQL = StringUtils.Format("INSERT INTO {0}.sfc_bomtaskitem(SFCBOMTaskID,BOMID,BOMItemID,MaterialID,"
						...
						+ "Remark,OperatorID,Operator,OperateTime,Quota,QualityLossBig,QualityLossSmall,ShiftID,ReplaceType,OutsourceType) "
						...
						+ ":OperatorID,:Operator,:OperateTime,:Quota,:QualityLossBig,:QualityLossSmall,:ShiftID,:ReplaceType,:OutsourceType);",
						wInstance.Result);
			} else {
				// 虚拟驳回时，修改操作时间
				...
				wSQL = StringUtils.Format(
						"UPDATE {0}.sfc_bomtaskitem SET SFCBOMTaskID = :SFCBOMTaskID,"
								...
								+ "ReplaceType=:ReplaceType,OutsourceType=:OutsourceType WHERE ID = :ID;",
						wInstance.Result);
			}
	...
	public double GetQuota(BMSEmployee wLoginUser, int partID, int partPointID, int materialID,
		...
		try {
			...
			String wSQL = StringUtils.Format("SELECT distinct MaterialNumber FROM {0}.mss_bomitem "
					+ "where ReplaceType=2 and MaterialID=:MaterialID and PlaceID=:PlaceID and PartPointID=:PartPointID;",
					wInstance.Result);
