...
public class APSAutoUtils {
	private static Logger logger = LoggerFactory.getLogger(APSAutoUtils.class);
...
//						} else {
						...
						// 判断此工位是否是已固定任务
						if (wMaualTaskPartMap.containsKey(wInteger)) {
							wOrderStartTime = (Calendar) wMaualTaskPartMap.get(wInteger).EndTime.clone();
							continue;
						}
						...
						if (wAPSTaskPart.PartID <= 0) {
							...
							if (wOptional.isPresent()) {
								...
								EditTaskTime(wLoginUser, wAPSTaskPart, wShiftPeriod, wRoutePartOrder, wOutsourceList,
										...
										wAPSDismantlingList, wWorkDay, true, wMessageList, wAllZoneList,
...
//			}
			Calendar wTaskEndTime = APSWorkTimeCalcUtils.getInstance().CalcTaskEndTime(wLoginUser, wTaskPart.PartID,
		...
		if (wIsMergeNext) {
			...
			for (APSTaskPart apsTaskPart : wNextList) {
				...
				if (wMaualTaskPartMap.containsKey(apsTaskPart.PartID))
					continue;
