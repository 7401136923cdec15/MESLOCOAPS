...
//				}
			...
			if (wBOMItem.ID <= 0) {
				wSQLText = StringUtils.Format(
						"INSERT INTO {0}.aps_bomitem ( BOMType, FactoryID, WBSNo, OrderID, PartNo, LineID, ProductID,CustomerID, PartID, "
								...
								+ "PickingQuantity,EvenExchangeRate,Client,OrderNum,SourceType,SourceID,DifferenceItem,OverQuota,SAPStatus,SAPMsg)  Values ( :BOMType, :FactoryID, :WBSNo, "
								...
								+ ":RepairCoreIdentification,:PickingQuantity,:EvenExchangeRate,:Client,:OrderNum,:SourceType,:SourceID,:DifferenceItem,:OverQuota,:SAPStatus,:SAPMsg)",
						wInstance.Result);
				// 重新赋值顺序
				...
				wSQLText = StringUtils.Format(
						"UPDATE {0}.aps_bomitem SET BOMType=:BOMType, PartID=:PartID, PartPointID=:PartPointID, MaterialID=:MaterialID, "
								...
								+ "Client=:Client,OrderNum=:OrderNum,SourceType=:SourceType,SourceID=:SourceID,DifferenceItem=:DifferenceItem,OverQuota=:OverQuota,SAPStatus=:SAPStatus,SAPMsg=:SAPMsg"
			...
			wParms.put("SAPStatus", wBOMItem.SAPStatus);
			wParms.put("SAPMsg", wBOMItem.SAPMsg);
		...
		try {
			...
			for (APSBOMItem wAPSBOMItem : wBOMItemList) {
				wValueStringList.add(StringUtils.Format(wValueTemp, wAPSBOMItem.BOMType, wAPSBOMItem.FactoryID,
						...
						wAPSBOMItem.DifferenceItem, wAPSBOMItem.OverQuota, wAPSBOMItem.SAPStatus, wAPSBOMItem.SAPMsg));
			}
			...
			String wSQLText = StringUtils.Format(
					"INSERT INTO {0}.aps_bomitem ( BOMType, FactoryID, WBSNo, OrderID, PartNo, LineID, ProductID,CustomerID, PartID, "
							...
							+ "EvenExchangeRate,Client,OrderNum,SourceType,SourceID,DifferenceItem,OverQuota,SAPStatus,SAPMsg)  Values {1}",
	...
	private List<APSBOMItem> APS_QueryBOMItemList(BMSEmployee wLoginUser, int wID, int wOrderID, String wWBSNo,
		...
		try {
			...
			for (Map<String, Object> wReader : wQueryResultList) {
				...
				wBOMItem.LineName = APSConstans.GetFMCLineName(wBOMItem.LineID);
				...
				wBOMItem.CustomerCode = APSConstans.GetCRMCustomer(wBOMItem.CustomerID).getCustomerCode();
				wBOMItem.PartID = StringUtils.parseInt(wReader.get("PartID"));
				wBOMItem.PartName = APSConstans.GetFPCPartName(wBOMItem.PartID);
				...
				wBOMItem.UnitText = APSConstans.GetCFGUnitName(wBOMItem.UnitID);
				...
				wBOMItem.Auditor = APSConstans.GetBMSEmployeeName(wBOMItem.AuditorID);
				...
				wBOMItem.Editor = APSConstans.GetBMSEmployeeName(wBOMItem.EditorID);
				...
				wBOMItem.SAPStatus = StringUtils.parseInt(wReader.get("SAPStatus"));
				wBOMItem.SAPMsg = StringUtils.parseString(wReader.get("SAPMsg"));
	...
	public APSBOMItem APS_SelectItem(BMSEmployee wLoginUser, int wOrderID, int wPartID, int wPartPointID,
			int wMaterialID, int wReplaceType, int wOutSourceType, OutResult<Integer> wErrorCode) {
		APSBOMItem wResult = new APSBOMItem();
		try {
			List<APSBOMItem> wList = this.APS_QueryBOMItemList(wLoginUser, -1, wOrderID, "", "", -1, -1, -1, wPartID,
					wPartPointID, wMaterialID, "", -1, wReplaceType, wOutSourceType, null, -1, -1, -1, wErrorCode);
	 ...
	 * 判断物料子项是否超定额了
	 */
	public String IsOverEqua(BMSEmployee wLoginUser, SFCBOMTask wTask, SFCBOMTaskItem wTaskItem,
			OutResult<Integer> wErrorCode) {
		String wResult = "";
		try {
			ServiceResult<String> wInstance = this.GetDataBaseName(wLoginUser.getCompanyID(), MESDBSource.Basic,
			...
			String wSQL = StringUtils.Format("select "
					+ "	(select count(Number)  from {0}.aps_bomitem where MaterialID=:MaterialID "
					+ "and PartID=:PartID and PartPointID=:PartPointID and OrderID=:OrderID and ReplaceType=2 and DeleteID != ''X'') Used,"
					+ "    (select MaterialNumber  from {0}.mss_bomitem where ID=:BOMItemID) Total;", wInstance.Result);

			Map<String, Object> wParamMap = new HashMap<String, Object>();

			wParamMap.put("MaterialID", wTaskItem.MaterialID);
			wParamMap.put("PartID", wTask.PartID);
			wParamMap.put("PartPointID", wTask.PartPointID);
			wParamMap.put("OrderID", wTask.OrderID);
			wParamMap.put("BOMItemID", wTaskItem.BOMItemID);
...
//				int wUsed = StringUtils.parseInt(wReader.get("Used"));
				// 根据订单、工位、工序、物料查询偶换件单据中的数量
				double wUsed = SFCBOMTaskItemDAO.getInstance().SelectUsedNumber(wLoginUser, wTask.OrderID, wTask.PartID,
						wTask.PartPointID, wTaskItem.MaterialID, wErrorCode);

				double wTotal = StringUtils.parseDouble(wReader.get("Total"));
				if (wUsed + wTaskItem.MaterialNumber > wTotal) {
					MSSMaterial wMaterial = GetMaterialByID(wLoginUser, wTaskItem.MaterialID, wErrorCode);
					wResult = StringUtils.Format("提示：{1}【{0}】超定额了。定额数：{2}，已领用或已申请领用：{3}，还需领用：{4}。",
							wMaterial.MaterialNo, wMaterial.MaterialName, wTotal,
							String.valueOf(wUsed).replace(".0", ""), wTaskItem.MaterialNumber);
				}
			}
		} catch (Exception ex) {
			logger.error(ex.toString());
	...
	public MSSMaterial GetMaterialByID(BMSEmployee wLoginUser, int wMaterialID, OutResult<Integer> wErrorCode) {
		...
		try {
			...
			String wSQL = StringUtils.Format(
					"select MaterialNo,MaterialName from {0}.mss_material where id=:wMaterialID;", wInstance.Result);
	...
	/**
	 * 判断指定订单，工位、工序，物料属性，物料是否存在
	 */
	public boolean IsExist(BMSEmployee wLoginUser, int wOrderID, int placeID, int partPointID, int materialID,
		...
		try {
			...
			String wSQL = StringUtils
					.Format("SELECT count(*) Number FROM {0}.aps_bomitem where OrderID=:OrderID and PartID=:PartID "
							+ "and PartPointID=:PartPointID and MaterialID=:MaterialID "
							+ "and ReplaceType=:ReplaceType and OutsourceType=:OutsourceType;", wInstance.Result);

			Map<String, Object> wParamMap = new HashMap<String, Object>();

			wParamMap.put("OrderID", wOrderID);
			wParamMap.put("PartID", placeID);
	 ...
	 * 打删除标记
	 */
	public void DeleteBOMX(BMSEmployee wLoginUser, int partID, int stepID, int materialID,
			OutResult<Integer> wErrorCode) {
		try {
			ServiceResult<String> wInstance = this.GetDataBaseName(wLoginUser.getCompanyID(), MESDBSource.Basic,
					wLoginUser.getID(), 0);
			...
			String wSQL = StringUtils.Format("update {0}.aps_bomitem set DeleteID=''X'' where PartID=:PartID "
					+ "and PartPointID=:PartPointID and SourceType=2 "
					+ "and DeleteID='''' and MaterialID=:MaterialID and ID>0;", wInstance.Result);
	 ...
	 * 查询偶换件关联的台车BOMID集合
	 */
	public int SelectIDListByBomTaskItemID(BMSEmployee wLoginUser, int wBomTaskItem, OutResult<Integer> wErrorCode) {
		int wResult = 0;
		try {
			ServiceResult<String> wInstance = this.GetDataBaseName(wLoginUser.getCompanyID(), MESDBSource.Basic,
					wLoginUser.getID(), 0);
			...
			String wSQL = StringUtils.Format("select ID from {0}.aps_bomitem where "
					+ "SourceID=:SourceID and SourceType=2 and DeleteID = '''';", wInstance.Result);

			Map<String, Object> wParamMap = new HashMap<String, Object>();

			wParamMap.put("SourceID", wBomTaskItem);
